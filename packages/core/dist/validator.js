...
export class Shader3DValidator {
    ...
    validateType(type) {
        ...
        if (this.options.strict && !/^[A-Z]/.test(type.name)) {
            this.addError('warning', `Type name '${type.name}' should start with uppercase`, type.location, 'W002', `Consider renaming to '${type.name.charAt(0).toUpperCase() + type.name.slice(1)}'`);
        }
    }
    validateField(field, structName) {
        // Validate type reference
        if (!this.isValidType(field.type)) {
            this.addError('error', `Unknown type '${this.typeToString(field.type)}' in field '${field.name}' of struct '${structName}'`, field.location, 'E001');
        }
    ...
    checkFieldAlignment(field, _index, type) {
        ...
        if (field.type.kind === 'vector' && field.type.size === 3) {
            this.addError('info', `vec3 in struct '${type.name}' has 16-byte alignment. Consider using vec4 or add padding.`, field.location, 'I001', 'WGSL aligns vec3 to 16 bytes, which may cause unexpected padding');
        }
    }
    validateShader(shader) {
        ...
        if (shader.returnType && !this.isValidType(shader.returnType.type)) {
            this.addError('error', `Unknown return type '${this.typeToString(shader.returnType.type)}' in shader '${shader.name}'`, shader.location, 'E002');
        }
    }
    validateVertexShader(shader) {
        // Must have @builtin(position) return or output
        const hasPositionOutput = shader.returnType?.attributes.some(a => a.name === 'builtin' && a.value === 'position') || shader.parameters.some(p => p.attributes.some(a => a.name === 'builtin' && a.value === 'position'));
        if (!hasPositionOutput && this.options.strict) {
            this.addError('warning', `Vertex shader '${shader.name}' should output @builtin(position)`, shader.location, 'W004');
        }
    }
    validateFragmentShader(shader) {
        // Should have @location(0) output
        const hasOutput = shader.returnType?.attributes.some(a => a.name === 'location');
        if (!hasOutput && this.options.strict) {
            this.addError('warning', `Fragment shader '${shader.name}' should have @location output`, shader.location, 'W005');
        }
    }
    validateComputeShader(shader) {
        // Must have workgroup_size
        const hasWorkgroupSize = shader.workgroupSize ||
            shader.attributes.some(a => a.name === 'workgroup_size');
        if (!hasWorkgroupSize) {
            this.addError('error', `Compute shader '${shader.name}' must have @workgroup_size attribute`, shader.location, 'E003', 'Add @workgroup_size(64) or similar');
        }
        // Performance: check workgroup size
        if (this.options.performance && shader.workgroupSize) {
            const total = shader.workgroupSize.reduce((a, b) => a * (b || 1), 1);
            if (total < 32) {
                this.addError('info', `Workgroup size ${total} is small. Consider using at least 64 for better GPU utilization.`, shader.location, 'P001');
            }
            if (total > 256) {
                this.addError('warning', `Workgroup size ${total} may exceed device limits on some GPUs`, shader.location, 'W006');
            }
        }
    }
    validateParameter(param, _shader) {
        // Check type validity
        if (!this.isValidType(param.type)) {
            this.addError('error', `Unknown type '${this.typeToString(param.type)}' for parameter '${param.name}'`, param.location, 'E004');
        }
        ...
        if (builtinAttr && builtinAttr.value && !WGSL_BUILTINS.has(builtinAttr.value)) {
            this.addError('error', `Unknown builtin '${builtinAttr.value}' for parameter '${param.name}'`, param.location, 'E005');
        }
        // Check location uniqueness (done in cross-validation)
    }
    validateGlobals(globals, _shaders) {
        const bindingMap = new Map();
        globals.forEach(global => {
            const key = `${global.group}-${global.binding}`;
            if (bindingMap.has(key)) {
                const existing = bindingMap.get(key);
                this.addError('error', `Duplicate binding @group(${global.group}) @binding(${global.binding})`, global.location, 'E006', `Previously defined at line ${existing.location?.line}`);
            }
        ...
        if (this.options.strict) {
            ...
            ast.gpuShaders.forEach(shader => {
                shader.parameters.forEach(p => {
                    if (p.type.kind === 'custom')
                        usedTypes.add(p.type.name);
                });
            ...
            ast.sharedTypes.forEach(type => {
                if (!usedTypes.has(type.name)) {
                    this.addError('info', `Type '${type.name}' is defined but never used`, type.location, 'I002');
                }
            });
        }
    }
    ...
    addError(severity, message, location, code = 'E000', suggestion) {
