...
export type TypeReference = {
    kind: 'primitive';
    name: 'f32' | 'i32' | 'u32' | 'bool' | 'number' | 'boolean';
} | {
    kind: 'vector';
    size: 2 | 3 | 4;
    elementType: 'f32' | 'i32' | 'u32' | 'bool';
} | {
    kind: 'matrix';
    rows: number;
    cols: number;
    ...
    kind: 'texture';
    textureType: 'texture_2d' | 'texture_3d' | 'texture_cube' | 'texture_storage_2d';
    sampleType?: string;
} | {
    kind: 'sampler';
    samplerType: 'sampler' | 'sampler_comparison';
} | {
    kind: 'pointer';
    addressSpace: 'function' | 'private' | 'workgroup' | 'uniform' | 'storage';
    elementType: TypeReference;
 ...
 */
export declare const BUILTIN_UNIFORMS: BuiltinUniform[];
/**
 * ShaderPark-style SDF primitives
 */
export type SDFPrimitive = 'sphere' | 'box' | 'torus' | 'cylinder' | 'cone' | 'plane' | 'capsule' | 'ellipsoid' | 'octahedron' | 'pyramid';
/**
 * SDF operations
 */
export type SDFOperation = 'union' | 'subtract' | 'intersect' | 'smoothUnion' | 'smoothSubtract' | 'smoothIntersect' | 'round' | 'onion';
/**
 * SDF transform functions
 */
export type SDFTransform = 'translate' | 'rotate' | 'rotateX' | 'rotateY' | 'rotateZ' | 'scale' | 'mirror' | 'twist' | 'bend';
