...
async function initCommand(name = 'shader3d-project', options) {
...
`.trim();
    ...
    fs.writeFileSync(path.join(projectDir, 'shader3d.config.json'), JSON.stringify(shader3dConfig, null, 2));
...
async function compileCommand(inputFile, options) {
    if (!inputFile) {
...
async function watchCommand(pattern, options) {
    if (!pattern) {
            ...
            try {
                ...
                if (mtime > last) {
                    ...
                    compileCommand(file, { ...options, verbose: false })
                        ...
                        .catch((e) => console.error('Compile error:', e.message));
                }
            }
            catch (e) {
...
async function analyzeCommand(inputFile, options) {
    if (!inputFile) {
...
async function bundleCommand(options) {
    ...
    for (const file of files) {
        try {
            const outputFile = path.join(outDir, path.relative('src', file).replace(/\.(ts|shader3d)$/, '.wgsl'));
...
async function upgradeCommand(_options) {
    console.log('\n⬆️  Checking for updates...\n');
    // This would typically use npm to check for updates
    console.log('Current version:', VERSION);
    console.log('To upgrade, run:');
    console.log('  npm update @shader3d/core @shader3d/runtime @shader3d/vite-plugin @shader3d/ladder');
}
function compileShader(source, options) {
