...
export class ProjectScaffolder {
    ...
    generateMinimalTemplate(options) {
...
<body>
  ...
  <script type="module" src="/src/main.${ext}"></script>
    ...
    generateViteVanillaTemplate(options) {
...
`
    ...
    generateViteReactTemplate(options) {
...
`
    ...
    generateParticlesTemplate(options) {
...
`;
    ...
    generateRaymarchingTemplate(options) {
        const files = this.generateViteVanillaTemplate(options);
        const ext = options.typescript ? 'ts' : 'js';
        files.find(f => f.path.includes('main.')).content = `
import { initWebGPU } from '@shader3d/runtime'
...
`;
    ...
    getShaderExample(level, typescript) {
...
`;
    ...
    getReactShaderComponent(level, typescript) {
