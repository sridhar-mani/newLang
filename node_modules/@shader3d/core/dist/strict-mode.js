...
export class StrictModeChecker {
    ...
    checkParameter(param, shader) {
        // Check for implicit any
        if (this.options.noImplicitAny) {
            if (param.type.kind === 'primitive' && param.type.name === 'number') {
                this.addError('error', `Parameter '${param.name}' has implicit 'number' type. Use explicit 'f32', 'i32', or 'u32'.`, param.location, 'S001');
            }
        }
        ...
        if (this.options.noUnusedParameters) {
            ...
            if (!bodyStr.includes(paramName)) {
                this.addError('warning', `Parameter '${param.name}' is declared but never used`, param.location, 'S002');
            }
        }
    }
    checkReturns(shader) {
        ...
        if (!hasReturn && shader.returnType) {
            this.addError('error', `Shader '${shader.name}' has return type but no return statement`, shader.location, 'S003');
        }
        // Check for early returns without value
        const emptyReturns = body.match(/\breturn\s*;/g);
        if (emptyReturns && shader.returnType) {
            this.addError('error', `Shader '${shader.name}' has empty return statement but expects ${this.typeToString(shader.returnType.type)}`, shader.location, 'S004');
        }
    }
    checkWorkgroupSize(shader) {
        const size = shader.workgroupSize || [64];
        const total = size.reduce((a, b) => a * (b || 1), 1);
        // Common limits
        if (total > 1024) {
            this.addError('error', `Workgroup size ${total} exceeds maximum of 1024`, shader.location, 'S005');
        }
        // Per-dimension limits
        size.forEach((dim, i) => {
            if (dim && dim > 1024) {
                const dims = ['X', 'Y', 'Z'];
                this.addError('error', `Workgroup dimension ${dims[i]} (${dim}) exceeds maximum of 1024`, shader.location, 'S006');
            }
        });
        // Recommend power of 2
        if (total > 0 && (total & (total - 1)) !== 0) {
            this.addError('info', `Workgroup size ${total} is not a power of 2. Consider using 64, 128, or 256 for better performance.`, shader.location, 'S007');
        }
    }
    checkBindings(shader) {
        ...
        shader.parameters.forEach(param => {
            ...
            if (!isBuiltin && !isLocation && !hasBinding) {
                ...
                if (param.type.kind === 'pointer' ||
                    ...
                    this.addError('error', `Resource parameter '${param.name}' requires @binding attribute`, param.location, 'S008');
                }
            }
        });
    }
    checkBody(shader) {
        ...
        if (this.options.noImplicitFloat) {
            ...
            if (floatLiterals && floatLiterals.length > 0) {
                this.addError('info', `${floatLiterals.length} float literal(s) without explicit 'f' suffix found`, shader.location, 'S009', 'Consider using 1.0f instead of 1.0 for clarity');
            }
        }
        ...
        if (this.options.noUnusedLocals) {
            ...
            for (const match of varDecls) {
                ...
                if (occurrences === 1) {
                    this.addError('warning', `Variable '${varName}' is declared but never used`, shader.location, 'S010');
                }
            }
        }
    ...
    checkAlignmentInBody(shader) {
        ...
        if (vec3ArrayPattern.test(body)) {
            this.addError('warning', `Array of vec3 detected. vec3 has 16-byte alignment which may cause padding issues.`, shader.location, 'S011', 'Consider using array<vec4> and ignoring the w component');
        }
    }
    typeToString(type) {
        switch (type.kind) {
    ...
    addError(severity, message, location, code = 'S000', suggestion) {
 ...
 */
export function checkStrictMode(ast, options) {
 ...
 */
export function getStrictModeDefaults(level) {
 ...
 */
export function parseStrictModeComment(comment) {
    const match = comment.match(/@shader3d-(strict|typed)(?:\s+(.+))?/);
    if (!match)
        return null;
    const options = { level: 'strict' };
    if (match[2]) {
