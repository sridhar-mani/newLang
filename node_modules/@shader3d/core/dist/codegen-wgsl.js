...
export class WGSLCodeGenerator {
    ...
    convertBodyToWGSL(body) {
        ...
        return cleaned
            ...
            // for loop conversion
            .replace(/for\s*\(\s*let\s+(\w+)\s*=\s*0\s*;\s*\1\s*<\s*(\w+)(?:\.length)?\s*;\s*\1\s*\+\+\s*\)/g, 'for (var $1: u32 = 0u; $1 < $2; $1++)');
    }
    emit(line) {
 ...
 */
export function generateWGSL(ast, options) {
 ...
 */
export async function compileToWGSL(source, filename, options) {
    // Import here to avoid circular dependency
    const { parse } = await import('./parser');
    const ast = parse(source, filename);
    return generateWGSL(ast, options);
}
//# sourceMappingURL=codegen-wgsl.js.map