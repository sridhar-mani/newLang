...
export class JSToWGSLConverter {
     ...
     */
    generateParameters(func, directive) {
        const params = [];
        if (directive.type === 'compute') {
     ...
     */
    generateReturnType(func, directive) {
    ...
    inferWGSLType(param, name) {
        ...
        if (param.type) {
            ...
            if (typeText.includes('Float32Array'))
                return 'ptr<storage, array<f32>, read_write>';
    ...
    convertBody(body, directive) {
        ...
        wgsl = wgsl
            ...
            .replace(/for\s*\(\s*(?:let|const|var)\s+(\w+)\s+of\s+(\w+)\s*\)/g, 'for (var __i: u32 = 0u; __i < arrayLength(&$2); __i++)')
            // Array length
 ...
 */
export function convertMagicToWGSL(source, filename) {
    const parser = new MagicCommentParser(source, filename);
